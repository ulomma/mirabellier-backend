...
function readCollectionCards(db, userId, { limit = 24, offset = 0, sort = "recent", search = "", element = "", minPower = 0, minGuard = 0, minSpeed = 0, minLuck = 0, duplicates = false } = {}) {
  ...
  const RARITY_CASE = `(CASE json_extract(cardJson, '$.rarity')
    WHEN 'UR' THEN 4
    WHEN 'SSR' THEN 3
    WHEN 'SR' THEN 2
    WHEN 'R' THEN 1
  ...
  const COLUMN_SQL = {
    ...
    "RH": `${RARITY_CASE} DESC`,
    "RL": `${RARITY_CASE} ASC`,
    "iv-desc": "json_extract(cardJson, '$.iv.total') DESC",
    "iv-asc": "json_extract(cardJson, '$.iv.total') ASC",
    "IH": "json_extract(cardJson, '$.iv.total') DESC",
    "IL": "json_extract(cardJson, '$.iv.total') ASC",
    "power-desc": "CAST(json_extract(cardJson, '$.iv.power') AS INTEGER) DESC",
    "guard-desc": "CAST(json_extract(cardJson, '$.iv.guard') AS INTEGER) DESC",
    "speed-desc": "CAST(json_extract(cardJson, '$.iv.speed') AS INTEGER) DESC",
    "effectHit-desc": "CAST(json_extract(cardJson, '$.iv.effectHit') AS INTEGER) DESC",
  };
...
function toPublicProfile(profile, equipmentStats, equippedItems, equipmentPctStats = {}, options = {}) {
  ...
  return {
    ...
    stats: {
      ...
      total: {
        hp: profile.hp + equipmentStats.hp + cardStats.hp + skillStats.hp + affinityStats.hp,
        power:
        ...
        effectHit: profile.effectHit + equipmentStats.effectHit + cardStats.effectHit + skillStats.effectHit + affinityStats.effectHit,
      },
    },
    selectedCard,
    canDrawCard,
...
function loadCombatSnapshot(db, profile, options = {}) {
  ...
  const cardStats = selectedCard
    ...
    : { hp: 0, power: 0, guard: 0, speed: 0, effectHit: 0 };
  const affinity = selectedCard && !profile.isNpc
    ? getCardAffinity(db, profile.userId, selectedCard.malId)
  ...
  return {
    ...
    baseStats: {
      hp: profile.hp + cardStats.hp + skillState.stats.hp + affinityStats.hp,
      power: profile.power + cardStats.power + skillState.stats.power + affinityStats.power,
      guard: profile.guard + cardStats.guard + skillState.stats.guard + affinityStats.guard,
      speed: profile.speed + cardStats.speed + skillState.stats.speed + affinityStats.speed,
      effectHit: profile.effectHit + cardStats.effectHit + skillState.stats.effectHit + affinityStats.effectHit,
    },
    totalStats: {
      hp: profile.hp + equipmentStats.hp + cardStats.hp + skillState.stats.hp + affinityStats.hp,
      power:
        profile.power +
