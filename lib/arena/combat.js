...
const { drawArenaCard } = require("../arena-characters");
const {
  nowIso, makeId, clamp, toInt, toPositiveInt, randomInt,
  getCurrentRecordedDate, xpToNext,
  ...
  RECENT_OPPONENT_LIMIT, toCombatName, getValueAtPath,
  CARD_IV_MAX,
} = require("./utils");
...
const NPC_TEMPLATES = [
  { id: "training-slime", displayName: "Training Slime", levelMax: 4, rarity: "C", ivRange: { min: 4, max: 14 }, statScale: 0.75 },
  { id: "shadow-pupil", displayName: "Shadow Pupil", levelMax: 9, rarity: "R", ivRange: { min: 6, max: 16 }, statScale: 0.78 },
  { id: "steel-paladin", displayName: "Steel Paladin", levelMax: 19, rarity: "R", ivRange: { min: 10, max: 22 }, statScale: 0.82 },
  { id: "arcane-knight", displayName: "Arcane Knight", levelMax: 34, rarity: "SR", ivRange: { min: 14, max: 26 }, statScale: 0.87 },
  { id: "dread-lord", displayName: "Dread Lord", levelMax: 49, rarity: "SR", ivRange: { min: 18, max: 30 }, statScale: 0.92 },
  { id: "celestial-warden", displayName: "Celestial Warden", levelMax: 64, rarity: "SSR", ivRange: { min: 20, max: 31 }, statScale: 0.95 },
  { id: "void-archon", displayName: "Void Archon", levelMax: 70, rarity: "UR", ivRange: { min: 22, max: 31 }, statScale: 0.98 },
];
const {
...
function loadCombatSnapshot(db, profile, options = {}) {
  ...
  const { equipped, stats: equipmentStats, pct: equipmentPctStats } = computeEquipmentStats(db, latestProfile.userId);
  const skillState = getSkillState(db, latestProfile);
  ...
  return {
    ...
    selectedCard: selectedCard ? attachCardAffinity(selectedCard, affinity) : null,
    rarity: selectedCard?.rarity || "C",
    baseStats: {
      hp: latestProfile.hp + cardStats.hp + skillState.stats.hp + affinityStats.hp,
      power: latestProfile.power + cardStats.power + skillState.stats.power + affinityStats.power,
      guard: latestProfile.guard + cardStats.guard + skillState.stats.guard + affinityStats.guard,
      speed: latestProfile.speed + cardStats.speed + skillState.stats.speed + affinityStats.speed,
      effectHit: latestProfile.effectHit + cardStats.effectHit + skillState.stats.effectHit + affinityStats.effectHit,
    },
    totalStats: {
      hp: latestProfile.hp + equipmentStats.hp + cardStats.hp + skillState.stats.hp + affinityStats.hp,
      power:
        latestProfile.power +
