...
const EFFECT_DURATION_LIMITS = Object.freeze({
  ...
  damageBoostFightsRemaining: 200,
  speedBoostFightsRemaining: 200,
  deathSaveCharges: 1000,
  statSteroidFightsRemaining: 1000,
  matchRarityCharges: 1500,
  vampiricHealFightsRemaining: 1000,
  critChanceBoostFightsRemaining: 1500,
  guardBoostFightsRemaining: 1500,
...
function computeMaxHp(stats, hpPct = 0) {
  ...
  const utilityBonus = Math.floor(
    (toInt(stats?.power, 0) + toInt(stats?.speed, 0)) * 0.35,
  );
...
function calculateAttackOutcome(input) {
  ...
  const defenseRoll =
    ...
    (Number(RARITY_CONFIG[defenderRarity]?.powerBonus || 0)) +
...
async function simulateFight(db, input) {
  ...
  const runAttack = (attackerSide, firstActor) => {
    ...
    if (elementMult > 1.0) {
      effectiveElementMult = 1.5 + toInt(attackerStats?.effectHit, 0) * 0.02;
      const reduction = toInt(defenderRuntime?.reduceElementEffectivenessPct, 0);
      if (reduction > 0) {
        effectiveElementMult = Math.max(1.5, effectiveElementMult * (1 - reduction / 100));
      }
    }
...
async function runFight(db, userId) {
  ...
  const tx = db.transaction(() => {
    ...
    if (current.level >= 5 && !current.tutorialComplete) {
      current.tutorialComplete = 1;
      current.coins += 10000;
      current.lifetimeCoinsEarned += 10000;
    }
