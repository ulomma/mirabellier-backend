...
function toPublicProfile(profile, equipmentStats, equippedItems, equipmentPctStats = {}, options = {}) {
  ...
  const hpBeforePct = profile.hp + equipmentStats.hp + cardStats.hp + skillStats.hp;
...
function getArenaProfilePayload(db, userId) {
  ...
  const equipmentPieces = getEquipmentPiecesRows(db, userId).map((row) => toPublicEquipmentPiece(row, true));
