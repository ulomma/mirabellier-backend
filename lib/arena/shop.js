...
function registerActiveConsumable(effects, item) {
  ...
  while (active.length >= MAX_ACTIVE_CONSUMABLE_EFFECTS) {
...
function pruneInactiveConsumables(effects) {
  ...
  effects.activeConsumables = effects.activeConsumables.filter((entry) => {
    const fields = ACTIVE_CONSUMABLE_EFFECT_FIELDS[entry.kind] || [];
    return fields.some((field) => toPositiveInt(effects[field], 0) > 0);
  });
}
...
function applyConsumableEffect(profile, item) {
  ...
  registerActiveConsumable(effects, item);
...
function useConsumable(db, userId, itemId) {
  ...
  const tx = db.transaction(() => {
    ...
    const nextEffects = item.type === "card"
      ...
      : applyConsumableEffect(profile, item);
    if (item.type === "card") {
